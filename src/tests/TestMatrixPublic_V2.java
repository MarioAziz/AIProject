...
public class TestMatrixPublic_V2 {
	...
	static class TH {
		...
		public TH(int m, int n, int x10, int x11, int x00, int x01, int m1, ArrayList<String> xyz, ArrayList<String> m4,
		...
		public boolean f42(int x, int y) {
			String ln = f1(x, y);
			if (this.xyz.contains(ln) || this.xyzw.contains(ln) || (this.m7.containsKey(ln) && this.m7.get(ln) > 97))
				return false;
			return true;
		}
		...
		public boolean grace() {
			return !this.ll && this.m3 < 100 && this.m7.size() == 0 && this.xyzw.size() == 0 && this.x00 == this.x10
					&& this.x01 == this.x11;
		}
	}
